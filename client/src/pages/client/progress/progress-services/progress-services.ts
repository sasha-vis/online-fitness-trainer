...
import { collection, addDoc, serverTimestamp, onSnapshot, query, orderBy, updateDoc, doc, deleteDoc, getDoc, where } from 'firebase/firestore';
...
export async function addMeasurement(data: Omit<BodyMeasurement, 'id' | 'createdAt'>): Promise<string> {
...
export function subscribeMeasurements(clientId: string, callback: (data: BodyMeasurement[]) => void) {
  const q = query(
