...
export const BodyMeasurementModal: React.FC<BodyMeasurementModalProps> = ({ open, onClose, onSubmit, loading, initialValues }) => {
  ...
  const handleOk = async () => {
    try {
      ...
      const photos: File[] = fileList.map(f => f.originFileObj).filter(Boolean) as File[]
      ...
      >
        {MEASUREMENT_FIELDS.map(field => (
          <Form.Item
            ...
            name={field.key}
            label={field.label}
            rules={[{ required: true, message: `Введите значение для "${field.label}"` }]}
          >
