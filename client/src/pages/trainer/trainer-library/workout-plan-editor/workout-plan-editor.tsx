...
export const WorkoutPlanEditor = () => {
  ...
  const addExercise = (workoutIndex, sectionIndex) => {
    ...
    const removeExercise = (workoutIndex, sectionIndex, exerciseIndex) => {
        const newPlan = JSON.parse(JSON.stringify(editingPlan));
        newPlan.workouts[workoutIndex].sections[sectionIndex].exercises.splice(exerciseIndex, 1);
        setEditingPlan(newPlan);
    };

    const updateExerciseField = (workoutIndex, sectionIndex, exerciseIndex, field, value) => {
        const newPlan = JSON.parse(JSON.stringify(editingPlan));
        newPlan.workouts[workoutIndex].sections[sectionIndex].exercises[exerciseIndex][field] = value;
        setEditingPlan(newPlan);
    };
  ...
  return (
        <div>
            <Card title={`Редактирование плана: ${editingPlan.goal}`} style={{ marginBottom: 24 }}>
                                ...
                                <Card 
                                    ...
                                    extra={
                                        <Button 
                                            ...
                                            danger 
                                            icon={<DeleteOutlined />}
                                            onClick={() => removeExercise(workoutIndex, sectionIndex, exerciseIndex)}
                                        />
                                    }
                                >
                                    <Space wrap style={{ width: '100%' }}>
                                        ...
                                        <InputNumber
                                            placeholder="Подходы"
                                            value={exercise.sets}
                                            onChange={(value) => updateExerciseField(
                                                workoutIndex, 
                                                sectionIndex, 
                                                exerciseIndex, 
                                                'sets', 
